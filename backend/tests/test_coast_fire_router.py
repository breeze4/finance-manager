...
SCENARIOS_URL = "/api/calculators/coast-fire/scenarios"


def _default_payload(name: str = "My plan", **overrides) -> dict:
    payload = {
        "name": name,
        "current_age": 30,
        "retirement_age": 65,
...
def test_create_returns_201_with_full_response(client: TestClient):
    ...
    assert data["current_savings"] == 50000
    assert data["expected_return_rate"] == 7
    assert data["target_retirement_amount"] == 1000000
    assert data["monthly_expenses"] == 0
    assert data["yearly_expenses"] == 0
    assert data["withdrawal_rate"] == 4
    assert data["inflation_rate"] == 0
    assert data["use_real_returns"] is False
    assert data["last_edited_field"] == "target"
...
def test_update_partial(client: TestClient):
    ...
    resp = client.put(
        ...
        json={"current_savings": 75000, "last_edited_field": "monthly"},
    )
    assert resp.status_code == 200
    data = resp.json()
    assert data["current_savings"] == 75000
    assert data["last_edited_field"] == "monthly"
...
def test_update_404(client: TestClient):
    resp = client.put(f"{SCENARIOS_URL}/99999", json={"current_savings": 100})
    assert resp.status_code == 404
...
def test_activate_switches_active_flag(client: TestClient, db: Session):
    ...
    rows = db.query(CoastFireScenario).filter(CoastFireScenario.is_active.is_(True)).all()
    assert len(rows) == 1
