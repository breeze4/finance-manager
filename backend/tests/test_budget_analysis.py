...
class TestHistoricalStats:
    def test_basic_average(self, db: Session):
        ...
        _make_txn(db, vendor="Store", amount=-400, txn_date=date(2025, 1, 15),
                  category_id=gid, import_hash="g1")
        _make_txn(db, vendor="Store", amount=-500, txn_date=date(2025, 2, 15),
                  category_id=gid, import_hash="g2")
        _make_txn(db, vendor="Store", amount=-600, txn_date=date(2025, 3, 15),
                  category_id=gid, import_hash="g3")
    ...
    def test_multiple_txns_per_month_sum(self, db: Session):
        ...
        _make_txn(db, vendor="Store A", amount=-200, txn_date=date(2025, 1, 5),
                  category_id=gid, import_hash="ga1")
        _make_txn(db, vendor="Store B", amount=-300, txn_date=date(2025, 1, 20),
                  category_id=gid, import_hash="ga2")
    ...
    def test_std_dev_calculation(self, db: Session):
        ...
        for i, amt in enumerate(amounts):
            _make_txn(db, vendor="Store", amount=-amt, txn_date=date(2025, i + 1, 15),
                      category_id=gid, import_hash=f"sd_{i}")
    ...
    def test_confidence_interval_unclamped(self, db: Session):
        ...
        for i, amt in enumerate(amounts):
            _make_txn(db, vendor="Store", amount=-amt,
                      txn_date=date(2025, (i % 12) + 1, 15),
                      category_id=gid, import_hash=f"wide_{i}")
    ...
    def test_transfers_excluded(self, db: Session):
        ...
        for i in range(6):
            _make_txn(db, vendor="Transfer", amount=-500, txn_date=date(2025, i + 1, 15),
                      category_id=tid, is_transfer=True, import_hash=f"t_{i}")
    ...
    def test_positive_amounts_excluded(self, db: Session):
        ...
        for i in range(6):
            _make_txn(db, vendor="Employer", amount=5000, txn_date=date(2025, i + 1, 1),
                      category_id=iid, import_hash=f"inc_{i}")
    ...
    def test_year_filter(self, db: Session):
        ...
        _make_txn(db, vendor="Store", amount=-400, txn_date=date(2025, 6, 15),
                  category_id=gid, import_hash="y25")
        _make_txn(db, vendor="Store", amount=-600, txn_date=date(2026, 1, 15),
                  category_id=gid, import_hash="y26")
    ...
    def test_sorted_by_average_descending(self, db: Session):
        ...
        for i in range(3):
            _make_txn(db, vendor="Big", amount=-1000, txn_date=date(2025, i + 1, 15),
                      category_id=cats["Shopping"], import_hash=f"big_{i}")
            _make_txn(db, vendor="Small", amount=-100, txn_date=date(2025, i + 1, 15),
                      category_id=cats["Dining"], import_hash=f"small_{i}")
...
class TestTrendDetection:
    def test_increasing_trend(self, db: Session):
        ...
        for i, amt in enumerate(amounts):
            _make_txn(db, vendor="Store", amount=-amt, txn_date=date(2025, i + 1, 15),
                      category_id=gid, import_hash=f"inc_{i}")
    ...
    def test_decreasing_trend(self, db: Session):
        ...
        for i, amt in enumerate(amounts):
            _make_txn(db, vendor="Store", amount=-amt, txn_date=date(2025, i + 1, 15),
                      category_id=gid, import_hash=f"dec_{i}")
    ...
    def test_stable_trend(self, db: Session):
        ...
        for i in range(6):
            _make_txn(db, vendor="Store", amount=-500, txn_date=date(2025, i + 1, 15),
                      category_id=gid, import_hash=f"flat_{i}")
    ...
    def test_few_months_stable(self, db: Session):
        ...
        _make_txn(db, vendor="Store", amount=-400, txn_date=date(2025, 1, 15),
                  category_id=gid, import_hash="few1")
        _make_txn(db, vendor="Store", amount=-600, txn_date=date(2025, 2, 15),
                  category_id=gid, import_hash="few2")
    ...
    def test_december_spike(self, db: Session):
        ...
        for i in range(11):
            _make_txn(db, vendor="Store", amount=-200, txn_date=date(2025, i + 1, 15),
                      category_id=sid, import_hash=f"norm_{i}")
        _make_txn(db, vendor="Store", amount=-800, txn_date=date(2025, 12, 15),
                  category_id=sid, import_hash="dec_spike")
...
class TestHistoricalAPI:
    def test_historical_endpoint(self, client: TestClient, db: Session):
        ...
        for i in range(6):
            _make_txn(db, vendor="Store", amount=-500, txn_date=date(2025, i + 1, 15),
                      category_id=gid, import_hash=f"api_{i}")
    ...
    def test_historical_with_year_filter(self, client: TestClient, db: Session):
        ...
        _make_txn(db, vendor="Store", amount=-400, txn_date=date(2025, 6, 15),
                  category_id=gid, import_hash="yf_25")
        _make_txn(db, vendor="Store", amount=-600, txn_date=date(2026, 1, 15),
                  category_id=gid, import_hash="yf_26")
